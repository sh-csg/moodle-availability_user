...
M.availability_user.form.initInner = function (param) {
    ...
    this.params.sort(function (a, b) {
        if (a.lastname.toLowerCase() < b.lastname.toLowerCase() || a.lastname.toLowerCase() == b.lastname.toLowerCase() && a.firstname.toLowerCase() == b.firstname.toLowerCase()) {
            return -1;
        }
...
M.availability_user.form.getNode = function (json) {
    ...
    var html = '<label><span class="col-form-label pr-3">' + strings.title + '</span><span class="availability-group form-group"><select class="custom-select">'

    this.params.forEach(val => html += '<option value="' + val.id + '">' + val.lastname + ', ' + val.firstname + (val.extra ? ' (' + val.extra + ')' : '') + '</option>');
...
M.availability_user.form.fillValue = function (value, node) {
    var user_select = node.one('select');
    value.userid = user_select.get('options').item(user_select.get('selectedIndex')).get('value');
};

M.availability_user.form.fillErrors = function (errors, node) {
};